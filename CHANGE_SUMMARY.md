...
## [0.9.0] - 2023-12-15
### 專案優化與 Rust 整合
- 將 Rust-order 目錄添加到 .gitignore，優化版本控制
- 更新前端界面組件，提升用戶體驗
- 添加新的頁面組件：AboutUsPage、FAQPage、PricingPage
- 優化前端組件結構，提高代碼可維護性

### 前端
- 添加 accordion 組件，增強內容展示能力
- 優化多個 UI 組件，提升視覺一致性
- 更新 bun 依賴管理，提高前端構建效率
- 完善頁面路由與導航結構

### 技術細節
- 更新 .gitignore 配置，排除 Rust 相關開發文件
- 優化前端組件庫，提高組件重用性
- 改進前端頁面結構，優化用戶導航體驗
- 更新批處理腳本，簡化開發與部署流程
...
### 前端界面優化
- 添加布局切換過場動畫，提升用戶體驗
- 實現類似 Gmail 的載入動畫，包含 Logo、進度條和動態背景效果
- 添加全局字體設置，使用 Noto Sans TC、思源黑體等優質中文字體
- 移除儀表板布局中的通知按鈕和側邊欄用戶資訊，簡化界面
- 優化頁腳布局和視覺層次，提升整體設計感
- 添加新的動畫效果，豐富視覺體驗

### 技術細節
- 創建 `MainLayout` 和 `DashboardLayout` 兩種布局組件，分離官網和功能區
- 實現智能布局切換檢測，僅在布局變更時顯示過場動畫
- 添加 `LoadingTransition` 組件，實現平滑的過場效果
- 優化字體系統，添加 CSS 變數和工具類
- 添加 Tailwind 動畫配置，支持更豐富的視覺效果
- 改進路由結構，優化用戶導航體驗
