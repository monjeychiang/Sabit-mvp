...
const PriceMonitorPage = () => {
  ...
  const fetchAvailableSymbols = async (marketType) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/price/symbols/${marketType}`);
  ...
  const handleSymbolFilter = (inputValue) => {
    ...
    Object.values(availableSymbols).forEach(symbols => {
      symbols.forEach(symbol => {
        if (!mergedSymbols.includes(symbol) && symbol.toLowerCase().includes(input)) {
    ...
    // 先顯示精確匹配的結果
    const exactMatches = mergedSymbols.filter(symbol => 
      symbol.toLowerCase().startsWith(input) || 
      symbol.toLowerCase().includes(`/${input}`)
    );
    
    // 再顯示部分匹配的結果
    const partialMatches = mergedSymbols.filter(symbol => 
  ...
  const fetchSubscriptions = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/price/subscriptions`);
      if (response.data.success) {
  ...
  const fetchAllPrices = async () => {
    ...
    for (const symbol of Object.keys(subscriptions)) {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/api/price/latest/${base}/${quote}`);
  ...
  const handleUnsubscribe = async (symbol, exchange = null) => {
    ...
    try {
      ...
      // 發送取消訂閱請求
      const response = await axios.post(`${API_BASE_URL}/api/price/unsubscribe`, requestData);
  ...
  useEffect(() => {
    ...
    if (autoRefresh && refreshInterval > 0 && Object.keys(subscriptions).length > 0) {
  ...
  return (
    <div className="container mx-auto py-6 px-4 sm:px-6">
      <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6">價格監控系統</h1>
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4 sm:space-y-6">
          <Card className="mb-6">
            ...
            <CardContent>
              <Form {...subscribeForm}>
                <form onSubmit={subscribeForm.handleSubmit(handleSubscribe)} className="space-y-4">
                  <FormField
                      ...
                      <FormItem>
                        <div className="flex justify-between items-center">
                          ...
                          <Badge variant="outline" className="text-xs">
                            ...
                            {": "}
                            {Object.values(availableSymbols).reduce((total, symbols) => total + symbols.length, 0)} 個交易對
                          </Badge>
                        </div>
                          ...
                          <div className="space-y-2">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="w-full">
                                  <div className="flex justify-between items-center mb-1">
                                    <p className="text-xs text-muted-foreground">下拉選單中已顯示所有交易對，可直接選擇或使用搜尋功能</p>
                                    <Badge variant="outline" className="text-xs bg-secondary/20">
                                      總計: {Object.values(availableSymbols).reduce((total, symbols) => total + symbols.length, 0)} 個交易對
                                    </Badge>
                                  </div>
                                  <Combobox
                                    items={filteredSymbols.map(symbol => ({ value: symbol, label: symbol }))}
                                    onValueChange={(value) => {
                                      console.log("Combobox 觸發選擇事件:", value);
                                      if (value) {
                                    ...
                                    emptyMessage={symbolInputValue ? "找不到符合的交易對" : "所有交易對已顯示，可直接選擇"}
                                  />
                                  {symbolInputValue && (
                                    <div className="text-xs text-muted-foreground mt-1 flex justify-between">
                                      <span>{filteredSymbols.length === 0 ? "找不到符合的交易對，請嘗試其他關鍵字" : `找到 ${filteredSymbols.length} 個符合的交易對`}</span>
                                    </div>
                                  )}
                                </div>
                                <div className="flex flex-col gap-1">
                                  <Button 
                                    ...
                                    variant="outline" 
                                    size="sm" 
                                    onClick={() => fetchAvailableSymbols(subscribeForm.getValues("marketType"), true)}
                                    disabled={loadingSymbols}
                                    className="whitespace-nowrap hover:bg-primary/10"
                                  >
                                    {loadingSymbols ? (
                                      <>
                                        <Loader2 className="mr-1 h-3 w-3 animate-spin" /> 載入中
                                      </>
                                    ) : (
                                      <>
                                        <RefreshCcw className="mr-1 h-3 w-3" /> 刷新列表
                                      </>
                                    )}
                                  </Button>
                                </div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2 p-2 bg-muted/30 rounded-md border border-dashed">
                              ...
                              {selectedSymbols.map(symbol => (
                                <Badge key={symbol} variant="secondary" className="px-3 py-1 hover:bg-secondary/80 transition-colors">
                                  {symbol}
                                  <button 
                                    type="button" 
                              ...
                              {selectedSymbols.length === 0 && (
                                <div className="text-sm text-muted-foreground flex items-center justify-center w-full py-4">
                                  <span className="mr-2">⚠️</span> 尚未選擇交易對，請從上方選擇
                                </div>
                              )}
                            </div>
                          </div>
                  ...
                  <Button type="submit" className="w-full bg-primary hover:bg-primary/90" disabled={isLoading}>
                    {isLoading ? (
                      <span className="flex items-center justify-center">
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" /> 訂閱處理中...
                      </span>
                    ) : (
                      <span className="flex items-center justify-center">
                        訂閱所選交易對
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="bg-muted/20 p-3 rounded-md">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {Object.entries(exchangeStatus).map(([key, connected]) => (
                      <div key={key} className="flex items-center p-2 bg-background rounded-md shadow-sm">
                        <Badge variant={connected ? "success" : "destructive"} className="mr-2">
                          {connected ? "已連接" : "未連接"}
                        </Badge>
                    ...
                    {Object.keys(exchangeStatus).length === 0 && (
                      <p className="text-muted-foreground text-sm p-2">暫無連接資訊</p>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-muted/20 p-3 rounded-md">
                  ...
                  <div className="flex items-center space-x-2 mb-3 p-2 bg-background rounded-md">
                    <div className="flex items-center space-x-2 flex-1">
                      ...
                      />
                      <label htmlFor="autoRefresh" className="text-sm">啟用自動刷新</label>
                    </div>
                    <Select
                ...
                <div className="flex space-x-2 justify-end">
                  <Button onClick={fetchServiceStatus} variant="outline" size="sm" className="text-xs">
                    <RefreshCcw className="mr-1 h-3 w-3" /> 刷新狀態
                  </Button>
                  <Button onClick={fetchAllPrices} variant="outline" size="sm" className="text-xs bg-primary/10">
                    手動刷新價格
                  </Button>
                </div>
              </div>
        ...
        <div>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Badge variant="outline" className="bg-primary/10 text-primary">{Object.keys(subscriptions).length} 個交易對</Badge>
            </CardHeader>
            <CardContent>
              {Object.keys(subscriptions).length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {Object.entries(subscriptions).map(([symbol, exchanges]) => (
                        exchanges.map(exchange => {
                          const price = priceData[symbol]?.prices?.[exchange];
                          const timestamp = priceData[symbol]?.timestamp;
                          const marketType = priceData[symbol]?.marketTypes?.[exchange] || 'spot';
                          ...
                          // 判斷是否為最近更新（30秒內）
                          const isRecentUpdate = timestamp && (Date.now() / 1000 - timestamp < 30);
                          
                          return (
                            <TableRow key={`${symbol}-${exchange}`} className="hover:bg-muted/20">
                              <TableCell className="font-medium">{symbol}</TableCell>
                              <TableCell>{exchange}</TableCell>
                              <TableCell>
                                <Badge variant="outline">{marketTypeText}</Badge>
                              </TableCell>
                              <TableCell className="font-semibold">
                                {price ? (
                                  <span className={isRecentUpdate ? "text-primary" : ""}>
                                    {price.toFixed(2)}
                                  </span>
                                ) : (
                              ...
                              <TableCell>
                                {timestamp ? (
                                  <span className={isRecentUpdate ? "text-primary text-xs" : "text-xs"}>
                              ...
                              <TableCell>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleUnsubscribe(symbol, exchange)}
