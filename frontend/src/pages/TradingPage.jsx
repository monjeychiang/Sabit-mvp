...
const TradingPage = () => {
  ...
  const fetchPositions = async (keyId) => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/exchange/${keyId}/positions`,
      );
  ...
  const fetchPositionMode = async (keyId) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/exchange/${keyId}/position-mode`,
      );
  ...
  const onOrderSubmit = async (data) => {
    ...
    try {
      ...
      if (
        ...
        if (symbol.includes("/")) {
          toast({
            title: "提示",
            description:
              "您正在合約模式下交易，但未使用合約交易對格式。系統將嘗試轉換格式。",
          });
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/exchange/${data.key_id}/orders`,
        payload,
      );
  ...
  const setLeverage = async (leverage) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/exchange/${selectedKeyId}/leverage`,
        payload,
      );
  ...
  const handleCancelOrder = async (orderId, symbol) => {
    ...
    try {
      await axios.delete(
        `${API_BASE_URL}/api/exchange/${selectedKeyId}/orders/${orderId}?symbol=${symbol}`,
      );
  ...
  const togglePositionMode = async () => {
    ...
    try {
      ...
      // 修正請求格式，確保參數名稱正確
      const response = await axios.post(
        `${API_BASE_URL}/api/exchange/${selectedKeyId}/position-mode`,
        {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="col-span-1">
          <CardHeader>
            ...
            <CardDescription>選擇要操作的交易所帳戶</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              >
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className="text-xs">
                          {tradingMode === "contract" &&
                            "請使用合約格式，如：BTC/USDT:USDT 或 BTC-PERP"}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    type="submit"
                    className={
                      tradingMode === "contract" ? "flex-1 ml-4" : "w-full"
                    }
      ...
      {tradingMode === "contract" && (
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>當前持倉信息</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
